...
{    
 ...
 int camera_init(char*ipStr)
{
  if(ipStr!=NULL)
    pRtpSession = rtpInit(ipStr,RTP_PORT); 
  else
  pRtpSession = rtpInit(SERVER_IP,RTP_PORT); 
  if(pRtpSession==NULL)    
     {    
 ...
 capture_init();
  ...
  {
 fprintf(stderr, "H264_Encode_init  err: %s \n", strerror(errno));
 return -1;
}
rtpSend(pRtpSession,oinfo.StrmVirAddr,14);
rtpSend(pRtpSession,oinfo.StrmVirAddr+14,9);
return 0;
}